...
export default function WatchPartyPage() {
  ...
  if (!configured) {
    return (
      <div className="rounded-2xl border border-white/[0.08] bg-[#141419] p-8 text-center">
        <h1 className="text-2xl font-bold text-white">Watch parties need Supabase Realtime</h1>
        <p className="mt-3 text-white/50">Configure Supabase URL, anon key, and service role key to create or join shared rooms.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <section className="rounded-2xl border border-white/[0.08] bg-[#141419] p-5 md:p-7">
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          ...
          <div className="grid gap-2 sm:grid-cols-[150px_1fr_auto]">
            <input value={code} onChange={(event) => setCode(event.target.value.replace(/\D/g, "").slice(0, 6))} className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-white outline-none focus:border-accent/60" placeholder="123456" />
            {!accountName ? (
              <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-white outline-none focus:border-accent/60" placeholder="Display name" />
            ) : (
              <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 py-3 text-sm text-white/70">{accountName}</div>
            )}
            <button onClick={() => join()} className="flex items-center justify-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-bold text-white hover:bg-accent-hover">
              <DoorOpen className="h-4 w-4" /> Join
            </button>
          </div>
        </div>
        {error ? <p className="mt-4 rounded-xl border border-red-500/20 bg-red-500/10 px-3 py-2 text-sm text-red-100">{error}</p> : null}
      </section>
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {rooms.map((room) => (
            <button key={room.id} onClick={() => join(room.id)} className="group overflow-hidden rounded-2xl border border-white/[0.08] bg-[#141419] text-left transition hover:border-accent/50 hover:bg-white/[0.04]">
