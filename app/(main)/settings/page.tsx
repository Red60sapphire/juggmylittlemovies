...
const SERVERS = ["VidLink", "Embed.su", "MultiEmbed", "VidSrc", "AutoEmbed", "VidBinge", "2Embed", "VidSrc 2", "VidSrc 3", "VidKing", "API Player"];
const DEFAULT_TABS = ["Home", "Movies", "TV Shows", "Anime", "Manga"];

function Toggle({ checked, onChange, label, description }: { checked: boolean; onChange: (v: boolean) => void; label: string; description?: string }) {
  return (
    <label className="flex items-center justify-between py-2.5 cursor-pointer group gap-4">
      <div>
...
export default function SettingsPage() {
  ...
  const moveServer = (index: number, dir: "up" | "down") => {
    const arr = [...settings.serverPriority];
    const swap = dir === "up" ? index - 1 : index + 1;
    if (swap < 0 || swap >= arr.length) return;
    [arr[index], arr[swap]] = [arr[swap], arr[index]];
    update({ serverPriority: arr });
  };
    ...
    <div className="mx-auto max-w-4xl space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card title="Playback" icon={<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}>
          ...
          <Toggle label="Autoplay previews on hover" checked={settings.autoplayPreviews} onChange={(v) => update({ autoplayPreviews: v })} />
          <Toggle label="Autoplay on cellular" checked={settings.autoplayOnCellular} onChange={(v) => update({ autoplayOnCellular: v })} description="Stream video automatically on mobile data" />
          <Toggle label="Data saver mode" checked={settings.dataSaverMode} onChange={(v) => update({ dataSaverMode: v })} description="Lower quality streams to save bandwidth" />
          <div className="border-t border-border/40 my-2" />
        ...
        <Card title="Content Providers" icon={<Wifi className="w-4 h-4" />}>
          ...
          <div className="space-y-1 max-h-48 overflow-y-auto scrollbar-hide">
            {settings.serverPriority.map((server, i) => (
              <div key={server} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.03] text-sm text-white/70">
                <span className="flex-1">{server}</span>
                <button onClick={() => moveServer(i, "up")} disabled={i === 0} className="p-0.5 text-muted hover:text-white disabled:opacity-30 transition-colors">▲</button>
                <button onClick={() => moveServer(i, "down")} disabled={i === settings.serverPriority.length - 1} className="p-0.5 text-muted hover:text-white disabled:opacity-30 transition-colors">▼</button>
              </div>
            ))}
          </div>
        </Card>
